(function () {
  ...
  function validateTicketForm(form) {
    ...
    if (existing && capacity + Number(input(form, "manual_reserve_capacity").value || 0) < Number(existing.sold || 0) + Number(existing.reserved || 0)) return "La suma del cupo online y manual no puede ser inferior a las entradas vendidas o reservadas.";
  ...
  function initScanner() {
    ...
    requireSession(function (sessionData) {
      ...
      function dietaryLabel(value) {
        return ({ vegetarian: "Vegetariana", vegan: "Vegana", pescatarian: "Pescetariana", other: "Necesidad especial" })[value] || "";
      }
      function foodAlertMarkup(attendee) {
        var alerts = [];
        if (Number(attendee.has_allergies) === 1 || attendee.has_allergies === true) {
          var allergyDetail = attendee.allergens || "Alergia comunicada";
          if (attendee.allergy_notes) allergyDetail += " · " + attendee.allergy_notes;
          alerts.push('<span><strong>⚠ ' + (Number(attendee.severe_allergy) === 1 || attendee.severe_allergy === true ? "ALERGIA GRAVE" : "ALERGIA") + '</strong> ' + escapeHtml(allergyDetail) + '</span>');
        }
        var diet = dietaryLabel(attendee.dietary_preference);
        if (diet) {
          var dietDetail = diet + (attendee.dietary_notes ? " · " + attendee.dietary_notes : "");
          alerts.push('<span><strong>⚠ MENÚ ESPECIAL</strong> ' + escapeHtml(dietDetail) + '</span>');
        }
        return alerts.length ? '<div class="ticket-food-alert"><strong>IDENTIFICAR CON PULSERA</strong>' + alerts.join("") + '</div>' : '<div class="ticket-food-status">✓ SIN ALERGIAS NI MENÚ ESPECIAL</div>';
      }
      ...
      function printGuestList() {
        ...
        var rows = guests.map(function (attendee, index) {
          ...
          return '<tr><td class="number">' + (index + 1) + '</td><td><strong>' + escapeHtml(attendee.name || "Sin nombre") + '</strong><small>' + escapeHtml(contact) + '</small>' + foodAlertMarkup(attendee) + '</td><td><strong>' + escapeHtml(attendee.ticket_type_name || "Entrada") + '</strong><small>' + escapeHtml(attendee.order_reference || "Sin referencia") + '</small></td><td><code>' + escapeHtml(attendee.public_code || "") + '</code></td><td>' + escapeHtml(presence) + '</td><td class="manual"><span class="check"></span><span class="line"></span></td></tr>';
        }).join("");
        printWindow.document.open();
        printWindow.document.write('<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Lista de invitados · ' + escapeHtml(eventName) + '</title><style>@page{size:A4 portrait;margin:12mm}*{box-sizing:border-box}body{margin:0;color:#17252a;font-family:Arial,sans-serif;font-size:9pt}header{display:flex;justify-content:space-between;gap:18px;padding-bottom:10px;border-bottom:2px solid #17252a}h1{margin:0 0 4px;font:700 22pt Georgia,serif}p{margin:0;color:#536166;line-height:1.4}.summary{margin:12px 0 10px;padding:8px 10px;background:#f0f3f1;font-size:8.5pt}.summary strong{color:#17252a}table{width:100%;border-collapse:collapse;table-layout:fixed}th{padding:7px 5px;border-bottom:1.5px solid #17252a;text-align:left;color:#405158;font-size:7pt;letter-spacing:.08em;text-transform:uppercase}td{padding:7px 5px;border-bottom:1px solid #b7c1c2;vertical-align:top;line-height:1.3}td strong,td small{display:block}td small{margin-top:2px;color:#5f6d71;font-size:7.5pt;overflow-wrap:anywhere}td code{font-family:"Courier New",monospace;font-size:7.5pt;overflow-wrap:anywhere}.ticket-food-alert{margin-top:5px;padding:4px 5px;border-left:3px solid #a82929;background:#fff1ed;color:#7d1616;font-size:7pt;line-height:1.35}.ticket-food-alert>strong{font-size:6.5pt;letter-spacing:.05em}.ticket-food-alert span{display:block;margin-top:2px}.ticket-food-status{margin-top:5px;color:#336747;font-size:6.5pt;font-weight:700}.number{width:4%;text-align:center}.manual{width:13%;white-space:nowrap}.check{display:inline-block;width:13px;height:13px;margin-right:8px;border:1px solid #17252a;vertical-align:middle}.line{display:inline-block;width:52px;border-bottom:1px solid #17252a;vertical-align:middle}footer{margin-top:10px;color:#617074;font-size:7.5pt}@media print{thead{display:table-header-group}tr{break-inside:avoid}footer{position:fixed;bottom:0;left:0;right:0}}</style></head><body><header><div><h1>Lista de invitados</h1><p>' + escapeHtml(eventName) + '</p></div><p>Impreso el<br><strong>' + escapeHtml(printedAt) + '</strong></p></header><div class="summary"><strong>' + guests.length + ' entradas activas</strong> · Las alertas rojas requieren identificar a la persona con pulsera.' + (excluded ? ' Se han excluido ' + excluded + ' entrada' + (excluded === 1 ? '' : 's') + ' anulada, reembolsada o bloqueada.' : '') + '</div><table><thead><tr><th class="number">#</th><th>Asistente</th><th>Entrada / pedido</th><th>Código</th><th>Estado actual</th><th class="manual">Control manual</th></tr></thead><tbody>' + rows + '</tbody></table><footer>Lista operativa de Perigallo · La validación definitiva debe registrarse después en el control de acceso.</footer></body></html>');
