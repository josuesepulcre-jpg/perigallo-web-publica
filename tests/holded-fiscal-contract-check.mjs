...
const cases = [
  ...
  ["queue derives production from the persisted order", () => assert.match(sync, /WHERE id = \? AND is_test = 0 AND environment = "production"[\s\S]*payment_status = "paid"/)],
  ["historical paid orders use either persisted payment field", () => assert.match(sync, /function isPaidOrder[\s\S]*payment_status.*paid/s)],
  ...
  ["document recovery uses an exact persisted reference", () => assert.match(sync, /findDocumentByReference\(\$order, \$documentType\)/)],
  ["tickets do not receive a duplicate payment", () => assert.match(sync, /\$documentType === 'salesreceipt' \? null/)],
  ["invoice payments remain recorded separately", () => assert.match(sync, /recordInvoicePayment\(\$documentId, \$payment\)/)],
  ["Holded invoices can be inspected during recovery", () => assert.match(client, /function invoices\(array \$filters = \[\]\)/)],
